...
BPF_LPM_TRIE(lpm_trie_map_f0, struct lpm_key_f0, u64, 1000000);

// Increment the count in the trie map if a match is found
static __always_inline int handle_event(__u32 input_ip) {
    struct lpm_key_f0 lookup_key = {.prefixlen = 32, .ip = input_ip};
    __u64 *lookup_value = lpm_trie_map_f0.lookup(&lookup_key);
    if (lookup_value) {
        ...
        lpm_trie_map_f0.update(&lookup_key, &temp_lookup_value);
        return 1;
    } else {
...
int xdp_prog_simple(struct xdp_md *ctx) {
    ...
    if (handle_event(iph->saddr) == 1) {
        ...
        __u32 port = 5;
        int redirect_result = bpf_redirect(port, 0);
	bpf_trace_printk("Ingress Port: %d\n",ctx->ingress_ifindex);
        bpf_trace_printk("Redirect result: %d\n", redirect_result);
        bpf_trace_printk("Redirect egress port: %d\n", port);
