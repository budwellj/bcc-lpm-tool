...
BPF_LPM_TRIE(lpm_trie_map_f1, struct lpm_key_f1, u64, 1000000);

// Increment the count in the trie map if a match is found
static __always_inline int handle_event(__u32 input_ip) {
    struct lpm_key_f1 lookup_key = {.prefixlen = 32, .ip = input_ip};
    __u64 *lookup_value = lpm_trie_map_f1.lookup(&lookup_key);
    if (lookup_value) {
        ...
        lpm_trie_map_f1.update(&lookup_key, &temp_lookup_value);
        return 1;
    } else {
